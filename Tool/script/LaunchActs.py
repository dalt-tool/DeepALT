...
class ActivityLauncher:
	...
	def testAPK(self, fileFolder, package, fn):
		...
		iccFile = fileFolder+os.sep+ "testCase.txt"
		...
		for line in lines:
			...
			if not os.path.exists(logcatInfo ):
				try:
					...
					executeCmd("adb shell am start -n dalt."+package +"/.Activity_"+ str(id)+" >> "+cmdInfo,5)
					...
					executeCmd("adb shell am force-stop dalt." +package)
	...
	def compareActivity(self, testActName, fn, package):
		result = os.popen('adb shell dumpsys activity activities |grep mResumed')
		res = result.read() 	
		str = ""
